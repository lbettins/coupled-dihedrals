...
def cubic_spline_interpolations(energy_dict,mode_dict):
    ...
    for mode in sorted(mode_dict.keys()):
        try:
            ...
            continue
        delta_q = delta_q_list[count]
        k = k_list[count]
        is_tors = is_tors_list[count]
        count += 1
...
def plot(polynomial_dict, energy_dict, mode_dict, mode):
    ...
    samples = sorted(energy_dict[mode].keys())
    print(samples)
    print(sorted(polynomial_dict[mode].keys()))
    ...
    from main import dict_to_NMode
    nmode = dict_to_NMode(mode,m_dict=mode_dict, e_dict=energy_dict, xyz_dict=None)
    q,V = nmode.spline(1000)
    f = nmode.get_spline_fn()
...
if __name__ == '__main__':
    ...
    plot = plot(polynomial_dict,energy_dict,mode_dict,mode=5)
