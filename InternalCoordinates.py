...
def get_RedundantCoords(atoms, cart_coords, 
        rotors_dict=None, imaginary_bonds=None):
    ...
    if imaginary_bonds is not None:
        for bond in imaginary_bonds:
            atom1, atom2 = bond
            # the index order in the pysisyphus package starts from 0
            new_imaginary_bonds.append([atom1-1, atom2-1]) 
            #
    ...
    def set_primitive_indices():
        ...
        if rotors_dict is not None and rotors_dict != []:
            ...
            for i in rotors_dict:
                ...
                scan_indices_set.add(tuple([scan[0]-1,scan[1]-1,scan[2]-1,scan[3]-1]))
    ...
    internal._prim_coords = np.array([pc.val 
        for pc in internal._prim_internals])
    return internal
#Shih-Cheng Li
